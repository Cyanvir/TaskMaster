...
主要特点：
- **工程化构建**：使用 CMake 进行项目管理。
- **模块化设计**：实现头文件与源文件的分离，职责清晰。
- **面向对象**：体现了封装 (Encapsulation) 和组合 (Composition) 关系。

## 2. 主要功能 (Features)
- [x] **添加任务**：支持输入自定义任务标题。
- [x] **查看列表**：显示当前所有任务及其完成状态。
- [x] **标记完成**：通过任务 ID 快速标记任务状态。
- [x] **修改任务**：支持对已存在的任务标题进行编辑 (包含输入验证)。
- [x] **全英文界面**：适配各类终端环境，避免编码乱码问题。
...
├── README.md           # 项目说明文档
├── .gitignore          # Git 忽略规则
├── pig_test.html       # 尝试GitHub配置网页，https://cyanvir.github.io/TaskMaster/pig_test.html网页链接
├── include/            # 头文件目录 (.h)
│   ├── Task.h          # Task 类声明
│   └── TaskManager.h   # TaskManager 类声明
└── src/                # 源文件目录 (.cpp)
    ├── main.cpp        # 程序入口与交互逻辑
    ├── Task.cpp        # Task 类实现
    └── TaskManager.cpp # TaskManager 类实现
...
# Mac/Linux
# ./TaskApp
没问题！这是完整版本的 README.md 内容。

你只需要点击代码块右上角的 “复制” (Copy) 按钮，然后把它粘贴到你项目根目录下的 README.md 文件里保存即可。

(记得把最后一行方括号里的内容改成你以后要传的象棋仓库链接，如果没有就先删掉那行)
...
Note: 除了本项目外，我正在开发一个更复杂的 C++ 课程设计项目：中国象棋 (Chinese Chess)。 欢迎访问我的另一个仓库查看：[这里填你象棋项目的GitHub链接，暂时没有可以先删掉这行]